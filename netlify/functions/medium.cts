...
export default async (request, context) => {
  try {
    ...
    const svgContent = await parseToMediumCard(requestObj, filteredResult); //becuz it's async, add await
...
async function parseToMediumCard(request, resultData) {
...
function getFirstImageFromHTML(html) {
  ...
  // Use querySelector to find the first image
  const firstImage = doc.querySelector("img");

  // Return the src attribute of the first image, or null if there are no images
...
const imgToDataURL = async (url) => {
  ...
  return parseImgToDataURL(fetchUrl);
};
...
const mediumCard = async (data, settings, index) => {
  ...
  // const blogImage = "data:image/png;base64," + atob(result);
  ...
  let border_color = defaultConfig.themes[selected_theme].border_color;

  let max_characters = 30;
  let character_tracker = 0;
  let line_tracker = 0;
  let max_lines = 2;
  var array_holder: any[] = [];
  var title_string = "";
  ...
  return `
    ...
    <defs>
      ...
      <pattern id="img${index}" patternUnits="userSpaceOnUse" x="0" y="0" width="100%" height="100%">
        <!-- image xlink:href="" x="${image.x}" y="${image.y}" height="${image.height}px" width="${image.width}px" /--> 
      </pattern>
    </defs>
